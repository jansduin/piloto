...
/**
 * Proveedor de Google Gemini para roles de ejecución (proposer, critic, verifier, chat).
 * Usa el modelo configurado en piloto.llm.google.models.execution.
 * Se registra como "google-execution" en el LLMProviderFactory.
 */
@Component
@ConditionalOnProperty(name = "piloto.llm.google.enabled", havingValue = "true")
public class GoogleGeminiExecutionProvider implements LLMProvider {
    private static final Logger logger = LoggerFactory.getLogger(GoogleGeminiExecutionProvider.class);
    ...
    public void init() {
        try {
            if (apiKey != null && !apiKey.isEmpty() && (projectId == null || projectId.isEmpty())) {
                logger.info("🔑 [Execution] Modo API Key de Google AI Studio activado. Modelo: {}", modelName);
                this.useRestApi = true;
            } else if (projectId != null && !projectId.isEmpty()) {
                logger.info("☁️ [Execution] Modo Vertex AI activado. Proyecto: {}, Modelo: {}", projectId, modelName);
                this.vertexAI = new VertexAI(projectId, location);
                this.model = new GenerativeModel(modelName, vertexAI);
                this.useRestApi = false;
            } else {
                logger.warn("⚠️ [Execution] No se detectó configuración válida (ApiKey o ProjectId).");
            }
        } catch (Exception e) {
            logger.error("❌ [Execution] Error inicializando GoogleGeminiExecutionProvider: {}", e.getMessage());
        }
    }
    ...
    public LLMResponse generateCompletion(LLMRequest request) {
        ...
        return LLMResponse.failure(getProviderName(), getModelName(), "Proveedor de ejecución no configurado correctamente.");
    }
        ...
        try {
            ...
            if (con.getResponseCode() == 200) {
                ...
                try (java.util.Scanner s = new java.util.Scanner(con.getErrorStream()).useDelimiter("\\A")) {
                    ...
                    logger.error("[Execution] Gemini API error {}: {}", con.getResponseCode(), errorBody);
                }
    ...
    private String extractTextFromJson(java.io.InputStream responseStream) {
        try {
            ...
            logger.warn("[Execution] No se pudo extraer texto de la respuesta Gemini: {}", root);
            return "Respuesta recibida sin contenido válido (ver logs)";
        } catch (Exception e) {
            logger.error("[Execution] Error parseando JSON de Gemini: {}", e.getMessage());
    ...
    public List<Float> generateEmbedding(String text) {
        throw new UnsupportedOperationException("Embeddings se gestionan via GeminiEmbeddingProvider.");
    }
